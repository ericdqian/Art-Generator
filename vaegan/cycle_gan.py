...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts, load):
...
def main(opts):
    ...
    if opts.run_all:
        ...
        init = True
        path = os.path.join(os.path.dirname(__file__), 'data', 'touse')
        for file in os.listdir(path):
            dataloader_X, test_dataloader_X = load_data(file, opts=opts)
            dataloader_Y, test_dataloader_Y = load_data(file, opts=opts)
            ...
            # Start training
            if init:
                training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts, False)
                init = False
            else:
                training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts, True)
    else:
        # Create train and test dataloaders for images from the two domains X and Y
        
            dataloader_X, test_dataloader_X = load_data(opts.X, opts=opts)
            dataloader_Y, test_dataloader_Y = load_data(opts.Y, opts=opts)
            ...
            # Start training
            training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts, opts.load)
...
def create_parser():
    ...
    parser.add_argument('--run_all', action = 'store_true', default = False, help = 'Choose whether or not to use all data or just Pointillism')
    ...
    # Data sources
    parser.add_argument('--X', type=str, default='Pointillism', choices=['Pointillism'], help='Choose the type of images for domain X.')
    parser.add_argument('--Y', type=str, default='Pointillism', choices=['Pointillism'], help='Choose the type of images for domain Y.')
