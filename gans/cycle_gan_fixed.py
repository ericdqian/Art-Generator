...
def save_samples(iteration, fixed_Y, fixed_X, G_YtoX, G_XtoY, opts):
    ...
    """
    fake_X = fixed_color_to_bw(fixed_Y)
...
def fixed_color_to_bw(imgs):
    ...
    for image in imgs:
        image = image.cpu()
        transform = transforms.Compose([transforms.ToPILImage(), transforms.Grayscale(3), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        image = transform(image)
        image = image.to(torch.device('cuda'))
        bw_images.append(image)
    new_images = torch.stack(bw_images)
    return new_images
...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        fixed_color_to_bw(images_Y)

        d_optimizer.zero_grad()

        D_X_loss = ((D_X(images_X) - 1)**2).sum()/len(images_X)
        d_real_loss = D_X_loss 
        ...
        g_loss = ((D_Y(fake_Y) - 1)**2).sum()/len(fake_Y)

        if opts.use_cycle_consistency_loss:
            reconstructed_X = fixed_color_to_bw(fake_Y)
...
def create_parser():
    ...
    # Data sources
    parser.add_argument('--X', type=str, default='bw', choices=['bw', 'color'], help='Choose the type of images for domain X.')
    parser.add_argument('--Y', type=str, default='color', choices=['bw', 'color'], help='Choose the type of images for domain Y.')
