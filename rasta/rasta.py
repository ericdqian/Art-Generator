...
def train_model(model, dataloader, criterion, optimizer, scheduler, use_gpu=False, num_epochs=25, log_dir="runs/logs/cnn"):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            writer.add_scalar('training/{}/acc'.format(phase), epoch_acc, epoch+1)
            save_checkpoint({
                'epoch': epoch + 1,
...
if __name__ == '__main__':
    ...
    wikiart_loader = WikiArtDataLoader(data_path, 9, (0.8, 0.1, 0.1), random_seed=42, num_workers=4, pin_memory=False)
    ...
    model = train_model(model, wikiart_loader, criterion, optimizer_ft, exp_lr_scheduler, use_gpu=use_gpu, num_epochs=50)
