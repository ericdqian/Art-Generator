...
def resnet_dropout(include_top=False, weights='imagenet', input_tensor = None, pooling='avg', input_shape=(224,224,3),classes=25,dp_rate=0.,n_retrain_layers=0):
    ...
    x = keras.applications.resnet50.conv_block(x, 3, [512, 512, 2048], stage=5, block='a')
    x = keras.applications.resnet50.identity_block(x, 3, [512, 512, 2048], stage=5, block='b')
    x = Dropout(dp_rate)(x)
    x = keras.applications.resnet50.identity_block(x, 3, [512, 512, 2048], stage=5, block='c')
